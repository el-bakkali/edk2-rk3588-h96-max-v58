...
{
  /*
   * No microSD socket on this board: sdmmc0 (fe2c0000) is disabled in the
   * vendor DT. GPIO4_PA5 must not be touched here - it is PCIe PERST.
   */
}
...
{
  switch (Segment) {
    case PCIE_SEGMENT_PCIE20L0: // soldered Wi-Fi/BT module
      /* reset */
      GpioPinSetDirection (4, GPIO_PIN_PA5, GPIO_PIN_OUTPUT);
      /* pcie_3v3 regulator enable */
      GpioPinSetDirection (4, GPIO_PIN_PA1, GPIO_PIN_OUTPUT);
      break;
    default:
      break;
  }
  ...
  )
{
  switch (Segment) {
    case PCIE_SEGMENT_PCIE20L0:
      GpioPinWrite (4, GPIO_PIN_PA1, Enable);
      break;
    default:
      break;
  }
  ...
  )
{
  STATIC CONST EFI_GUID  VendorDtbFileGuid = {
    ...
    0x05a2c2ff, 0xfc85, 0x4323, { 0x8e, 0x2f, 0x7e, 0x42, 0x31, 0xf5, 0x88, 0x2e }
  };
  STATIC CONST EFI_GUID  MainlineDtbFileGuid = {
    // DeviceTree/Mainline.inf
    0xdccf6ea6, 0xd5a3, 0x44fa, { 0x94, 0x27, 0x73, 0x29, 0xf9, 0xa0, 0x4c, 0xe3 }
  };
